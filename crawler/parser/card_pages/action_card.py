...
class ActionCardParser(AbstractParser):
    def __init__(
        self,
        *args,
        config,
    ...
    async def _parse(
        ...
        base_info_element = element.locator(
            'div#module-1.obc-tmpl-part-wrap.obc-tmpl-part--align-left, '
            'div#module-85.obc-tmpl-part-wrap.obc-tmpl-part--align-left'
        ).first
        ...
        general_skill_element = element.locator(
            'div#module-5840.obc-tmpl-part-wrap.obc-tmpl-part--align-left'
        ).first
        ...
        # 效果描述
        effectiveness_element = element.locator(
            'div#module-2.obc-tmpl-part-wrap.obc-tmpl-part--align-right, '
            'div#module-86.obc-tmpl-part-wrap.obc-tmpl-part--align-right'
        ).first
        if await element_exists(effectiveness_element):
            res_info['效果描述'] = await self._parse_effectiveness(
                context_page=context_page,
                browser_context=browser_context,
                element=effectiveness_element,
            )

        # 卡牌故事
        card_story_element = element.locator(
            'div#module-3.obc-tmpl-part-wrap.obc-tmpl-part--align-right, '
            'div#module-87.obc-tmpl-part-wrap.obc-tmpl-part--align-right'
        ).first
        check = card_story_element.locator(
            'div.obc-tmpl-fold.obc-tmpl-part.obc-tmpl-collapsePanel'
        ).first
        if await element_exists(check):
            res_info['卡牌故事'] = await self._parse_card_story(
                context_page=context_page,
                browser_context=browser_context,
